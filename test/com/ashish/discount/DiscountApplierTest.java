...
public class DiscountApplierTest {
	...
	public void discountForMacAndIphoneTogether() {
		...
		Assert.assertEquals(expected, basket.getAmount(), 0.001);
	}
	...
	public void discountFor3or4Products() {
		...
		Basket basket = new BasketBuilder().w("REFRIGERATOR", 2000).w("DISH WASHER", 500).w("TRASH CAN", 2500).build();
	...
	public void discountForMoreThan5Products() {
		...
		Basket basket = new BasketBuilder().w("REFRIGERATOR", 2000).w("DISH WASHER", 500).w("TRASH CAN", 2500)
				.w("BED", 400).w("BED", 200).build();
